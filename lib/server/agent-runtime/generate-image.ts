...
/**
 * Persist generated bytes through the shared asset registry — the same neutral
 * surface `use_material_media` and the TTS path use. The returned id is the
 * stable `src` the agent later writes into a slide media element; the renderer
 * resolves registry ids to serving URLs at display time.
 */
export async function defaultPersistGeneratedImage(
  input: PersistImageInput,
  assetStore?: Pick<AssetStore, 'put'>,
): Promise<string> {
  const { bytes, mime } = await imageBytes(input.result, input.signal);
  ...
  throwIfAborted(input.signal);
  if (!src) throw new Error('Image storage returned an empty URL');
  return src;
}
