...
const stableRevisionedTargetRegistry = new RevisionedWhiteboardTargetRegistry();
const readRevisionedStageId = () => useStageStore.getState().stage?.id;
const readRevisionedSceneId = () => useStageStore.getState().currentSceneId;
...
function parseMutationResult(context: Context): {
...
function batch2Arguments(toolName: Batch2ToolName) {
  switch (toolName) {
    case 'wb_draw_latex':
      return { latex: '\\frac{x}{2}', x: 40, y: 50, width: 300, height: 80 };
    case 'wb_draw_table':
      return {
        data: [
          ['<b>&', 'Value'],
...
afterEach(() => {
  ...
  stableRevisionedTargetRegistry.clear();
...
describe('Stage 3B-2 real same-Child continuation', () => {
  it('runs wb_read → stale draw → wb_read → committed draw → final text through StreamBuffer', async () => {
    ...
    const queryRuntime = new BrowserClientQueryRuntime({
      requestId: 'request-1',
      sessionId: 'session-1',
      readCurrentStageId: readRevisionedStageId,
      readCurrentSceneId: readRevisionedSceneId,
    ...
    const revisionedBrowser = new BrowserRevisionedWhiteboardEffectRuntime({
      ...
      targetRegistry: stableRevisionedTargetRegistry,
  ...
  it('runs wb_read → Shape v2 → Line v2 → final text in one Child and two actions', async () => {
    const initialStage = stage();
    vi.stubEnv('OPENMAIC_ENABLE_PI_NATIVE_CHILD_RUNTIME', 'true');
    vi.stubEnv('OPENMAIC_ENABLE_PI_NATIVE_CHILD_WHITEBOARD', 'true');
    setStageStoreStateThroughAuthority({ stage: initialStage, currentSceneId: 'scene-1' });
    ...
    if (!before.ok) throw new Error('Expected authoritative state.');
    ...
    const buffer = new StreamBuffer(
      {
        ...
        onActionReady() {},
        onRevisionedClientEffectQueued(delivery) {
          lifecycle.push(`queued:${delivery.toolName}`);
          revisionedBrowser.reserve(delivery);
        },
        async onRevisionedClientEffectReady(_messageId, delivery, signal) {
          lifecycle.push(`execute:${delivery.toolName}`);
          await revisionedBrowser.execute(delivery, signal);
        },
        onLiveSpeech() {},
        onSpeechProgress() {},
    ...
    buffer.pushAgentStart({ messageId: 'child-1', agentId: 'teacher-1', agentName: 'Teacher' });
    buffer.start();
    ...
    const shape = buildInternalRevisionedWhiteboardDrawShapeTool(common);
    const line = buildInternalRevisionedWhiteboardDrawLineTool(common);
    ...
    const streamFn = ((_model, context) => {
      ...
      if (turn === 1) {
        return stream(
          assistant(
            [{ type: 'toolCall', id: 'read-1', name: 'wb_read', arguments: { scope: 'summary' } }],
            'toolUse',
          ),
        );
      }
      ...
      if (turn === 3) {
        const shapeResult = parseMutationResult(context);
        return stream(
          assistant(
            [
              {
                ...
                name: 'wb_draw_line',
                arguments: {
                  observationToken: shapeResult.observationTokens.bindingObservationToken,
                  expectedWhiteboardId: shapeResult.currentBinding.whiteboardId,
                  expectedRevision: shapeResult.currentBinding.revision,
                  startX: 100,
                  startY: 100,
                  endX: 300,
                  endY: 220,
      ...
      expect(latestToolText(context)).toContain('Whiteboard mutation result');
      return stream(
        assistant(
          [{ type: 'text', text: 'Shape and line were committed in the same Child.' }],
          'stop',
        ),
      );
    }) as StreamFn;
    const executionIds = ['query-1', 'shape-execution', 'line-execution'];
    const result = await runNativeChild({
      traceId: 'trace-1',
      runId: 'run-1',
      agentInvocationId: 'child-1',
      ...
      streamFn,
      systemPrompt: 'Use authoritative whiteboard reads and revisioned mutations.',
      prompt: 'Draw a shape and connect it with a line.',
      tools: [read.tool, shape.tool, line.tool],
      allowedToolNames: new Set(['wb_read', 'wb_draw_shape', 'wb_draw_line']),
      clientQueryHandlers: new Map([['wb_read', read.handler]]),
      clientEffectHandlers: new Map([
        ['wb_draw_shape', shape.handler],
        ['wb_draw_line', line.handler],
      ]),
      toolCategories: new Map([
        ['wb_read', 'read'],
        ['wb_draw_shape', 'mutation'],
        ['wb_draw_line', 'mutation'],
      ]),
      toolBudgets: {
        maxMutationExecutions: 2,
        maxReadExecutions: 8,
    ...
    expect(result.status).toBe('completed');
    expect(result.finalOutput).toBe('Shape and line were committed in the same Child.');
    expect(result.toolBudgetUsage).toEqual({
      mutationExecutions: 2,
      readExecutions: 1,
      otherToolExecutions: 0,
    ...
    expect(lifecycle).toEqual([
      'queued:wb_draw_shape',
      'execute:wb_draw_shape',
      'queued:wb_draw_line',
      'execute:wb_draw_line',
    ]);
    expect(errors).toEqual([]);
    expect(common.onActionDone).toHaveBeenCalledTimes(2);
    expect(authority.querySnapshot()).toMatchObject({
      ok: true,
      value: { revision: before.value.revision + 2, open: true },
    });
    expect(
      useStageStore.getState().stage?.whiteboard?.[0].elements.map(({ type }) => type),
    ).toEqual(['shape', 'line']);
  });

  it.each(['wb_draw_latex', 'wb_draw_table', 'wb_draw_chart'] as const)(
    'runs stale %s v2 → fresh wb_read → retry → final text in one real Child',
    async (toolName) => {
      const initialStage = stage();
      vi.stubEnv('OPENMAIC_ENABLE_PI_NATIVE_CHILD_RUNTIME', 'true');
      vi.stubEnv('OPENMAIC_ENABLE_PI_NATIVE_CHILD_WHITEBOARD', 'true');
      ...
      const mutation =
        toolName === 'wb_draw_latex'
          ? buildInternalRevisionedWhiteboardDrawLatexTool(common)
          : toolName === 'wb_draw_table'
            ? buildInternalRevisionedWhiteboardDrawTableTool(common)
            : buildInternalRevisionedWhiteboardDrawChartTool(common);
      ...
      const streamFn = ((_model, context) => {
        ...
        if (turn === 2 || turn === 4) {
          ...
          if (!advanced) {
            ...
            expect(
              authority.transact({
                label: 'test.batch2-external-authority-write',
                writes: [
                  {
                    label: 'create-board',
                    write: () =>
                      useStageStore.setState({
                        stage: { ...current, whiteboard: [externalBoard] },
                      }),
                  },
                ],
                preferredActiveWhiteboardId: externalBoard.id,
          ...
          return stream(
            assistant(
              [
                {
                  type: 'toolCall',
                  id: turn === 2 ? 'mutation-stale' : 'mutation-fresh',
                  name: toolName,
                  arguments: {
                    observationToken: readResult.observationTokens.bindingObservationToken,
                    expectedWhiteboardId: readResult.whiteboardId,
                    expectedRevision: readResult.revision,
                    ...batch2Arguments(toolName),
                  },
                },
              ],
              'toolUse',
        ...
        expect(latestToolText(context)).toContain('Whiteboard mutation result');
        return stream(
          assistant([{ type: 'text', text: `${toolName} continued in the same Child.` }], 'stop'),
        );
      }) as StreamFn;
      const executionIds = [
        'query-stale',
        `${toolName}-stale-execution`,
        'query-fresh',
        `${toolName}-fresh-execution`,
      ];
      const result = await runNativeChild({
        traceId: 'trace-1',
        runId: 'run-1',
        ...
        streamFn,
        systemPrompt: 'Use authoritative whiteboard reads and revisioned mutations.',
        prompt: `Execute ${toolName} after reading current state.`,
      ...
      expect(result.status).toBe('completed');
      expect(result.finalOutput).toBe(`${toolName} continued in the same Child.`);
      expect(result.toolBudgetUsage).toEqual({
        mutationExecutions: 2,
        readExecutions: 2,
        otherToolExecutions: 0,
      ...
      expect(useStageStore.getState().stage?.whiteboard?.[0].elements[0]?.type).toBe(
