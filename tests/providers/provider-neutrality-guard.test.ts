...
const TEMPORARY_ALLOWLIST: readonly AllowedLeak[] = [
  ...
  {
    file: 'lib/server/agent-runtime/agent-driver-model.ts',
    vendor: 'openai',
    token: 'OPENAI_PI_APIS',
    count: 2,
    reason: 'Temporary: the driver transport dialect capability is not declared by the provider.',
  },
  {
    file: 'lib/server/agent-runtime/agent-driver-model.ts',
    vendor: 'openai',
    token: "'openai-completions'",
    count: 1,
    reason: 'Temporary: the driver transport dialect capability is not declared by the provider.',
  },
  {
