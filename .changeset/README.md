...
So it is granted only to a range that **cannot** contain anything else. Two conditions:

- nothing it deletes loses a merged release intent. Every package a deleted changeset named must
  either be released here, or be named again by a changeset this range adds
- every file it touches is a `CHANGELOG.md`, a deleted changeset, or a manifest edited the way
  `changeset version` edits one — the `version` field, and the range of a dependency on a sibling
  in this family. A hand-written version bump sitting next to a real source edit does not qualify,
  and neither does a manifest whose `files`, `exports`, `scripts` or third-party dependencies
  moved, because each of those decides what the tarball contains

The first condition is checked on its own, so a range that legitimately declares its own change
cannot quietly drop somebody else's pending release on the way past. If you need to withdraw a
pending release — the change it described was reverted, say — name the package in a changeset of
your own, or let the release go out.
...
Four things this scheme does not prove. Each is a decision about how the packages are built or
released rather than something the merge-time check can settle, and stating them is better than
implying a guarantee that is not there.

**Root-level build inputs.** "Changed" means "a file under the package directory changed", which
under-approximates the real inputs of all four packages. `renderer` and `importer` inline their
dependency graph through Rollup, so a lockfile-only resolution change rewrites their published
bundles. `dsl` and `storage` are not exempt either: their `dist` is whatever the lockfile's
TypeScript emits, and `dsl`'s shipped JSON Schema is generated by the lockfile's
`ts-json-schema-generator`. A toolchain bump can therefore change any of the four tarballs with no
diff under the package directory. Closing this would mean treating the lockfile and toolchain
configuration as an input of every package, which makes every dependency bump demand four
changesets, or externalising the bundled dependencies.

**A later `files` whitelist change.** The per-package ignore lists in `check-changesets.mjs`
record what each package does not ship _today_ — `test/`, `docs/`, `vitest.config.ts`, the
importer's `src1/`. Adding one of those to a package's `files` would start shipping it while the
check still treats it as free. Nothing detects that on its own, so the lists have to be revisited
whenever a `files` whitelist grows.

**`workspace:^` deduplicates within a version line only.** Publishing the family with caret ranges
means a consumer on one dsl line installs one copy. A consumer that mixes an older dependent
needing `^0.5.0` with a newer one needing `^0.6.0` still resolves two dsl copies. Only a
coordinated major across the family, or making dsl a peer dependency, would prevent that.

**The level is a human judgement.** The check enforces that a changed package is declared and that
the level releases something. It cannot read a diff and decide what it does to consumers, so
whether a breaking `dsl` change is declared `minor` rather than `patch` is decided in review.
