...
The key domain has **no length constraint** — a `stageId` is a string of any length, so any tight bound would eventually reject a legitimate `prefix + id`. Backends MAY apply a generous denial-of-service ceiling so a single key cannot blow out a URL or a server index, but it is a resource guard, not part of the key domain, and MUST sit far above any identifier a caller could realistically produce.

Any such ceiling MUST be measured on the key's **percent-encoded length** (the size it occupies as a request target), not its UTF-8 byte length. A key travels as a URL path segment, and a non-ASCII character expands when encoded — `€` is three UTF-8 bytes but `%E2%82%AC`, nine, on the wire. A byte-length bound therefore lets a key pass client validation and then exceed the HTTP server's request-target limit, which the server rejects with `431` *before routing*, while a browser backend — which never builds a URL — accepts it: a silent backend-parity break. Bounding the encoded size keeps both backends in step: a key is accepted exactly when its encoded form fits, on whichever backend it reaches. The backends here cap at 4096 encoded bytes — far above a real id (a 500-character id under `editor-current-scene:` encodes to ~523) and well below the ~16 KiB request-target limit, so no key that passes validation can ever reach the `431`.
