...
Stage cascade deletion is an admin-plane capability. If exposed to ordinary learners it can erase every partition on a stage, so it is controlled by a separate admin authorization hook and denied by default. Production systems should isolate admin credentials, audit decisions, protect against confused-deputy use, and avoid deriving admin authority from a learner-controlled claim.
