...
- **The `302` itself carries `X-Asset-Revision`**, `Cache-Control: private, no-store`, and `Vary: Cookie, Authorization, X-Asset-Egress` — the read route's posture, since the redirect is as per-principal as the bytes it points at. A client whose `fetch` follows redirects opaquely observes only the final response's pinned headers, which is one reason the packaged client asks for the descriptor below rather than follow.
...
What a conforming client does about the hidden `302` headers. Following a redirect is not neutral for it: the platform fetch forwards the original request's headers to the object store's origin — only `Authorization` is stripped across origins — so a deployment whose credential travels in a custom header would hand that credential to the object store, and the cross-origin preflight the forwarded custom headers provoke commonly fails there besides. A client therefore asks for the descriptor instead: it sends `X-Asset-Egress: descriptor` on every byte `GET`, and a redirect-egress server answers `200` with a JSON body `{"url": ..., "revision": ...}`, the `X-Asset-Egress: descriptor` response header, and the read route's cache posture — never a `Location`. The client then fetches the signed URL itself, with no deployment headers at all, takes the revision from the descriptor and the served media type from the byte response, and is otherwise unchanged: warm reads still revalidate through `HEAD`, which is never answered indirectly. The descriptor answer is distinguishable from direct bytes by its response header alone, because an asset whose own media type is `application/json` must not parse as one. A server that never redirects ignores the request header and answers bytes exactly as before, so one client build serves both deployment shapes. The `302` remains the answer for generic HTTP consumers that did not ask for the descriptor; what they do with their own headers is their own affair.
