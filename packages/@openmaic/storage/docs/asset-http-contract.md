...
Everything so far describes **direct** egress: a byte `GET` is answered `200` with the bytes materialized in the response body. A deployment MAY instead opt into **indirect** egress, under which a byte `GET` is answered `302 Found` with a `Location` carrying a short-lived signed URL for the object — when, and only when, the byte layer can sign. This is the signed object-storage shape anticipated above, and it is additive in the precise sense that a deployment which never opts in cannot tell it exists: off by default, and byte-for-byte unchanged when off. Only the byte `GET` changes shape; `HEAD`, writes, and `remove` are always answered directly, so `X-Asset-Revision` revalidation through `HEAD` works exactly as under direct egress.

Indirect egress is a preference, not a guarantee. A byte layer that cannot sign — the byte column in the transactional store cannot — answers `200` with the bytes even when the deployment opted in, and a client MUST therefore handle both shapes on the same route. In practice it handles them identically without knowing which it got: its ordinary `fetch` follows the redirect automatically, and the signed URL wants no credentials, so the follow-up request carries none of the deployment's own.
...
What a conforming client does about the hidden `302` headers. A `fetch` that follows the redirect observes only the object store's response, which carries the pinned labels but no revision, so a cold read cannot take its identity from the byte response the way direct egress allows. The required shape is: detect the situation once — a byte response that followed a redirect and carries no revision — and from then on probe `HEAD` before each cold `GET`, taking the label from the probe. The order is the substance, not a nicety: the probe predates the download, so a replacement in between labels newer bytes with an older revision, which the next revalidation detects and corrects; probing after the download could pin stale bytes under a fresh revision with no later signal to repair it. A client that has not yet observed a redirect keeps the direct behavior byte-for-byte, and a probe that finds `404 ASSET_NOT_FOUND` answers the miss without a download. One consequence a deployment should know: the first cold read per client under indirect egress downloads the bytes twice, once to learn the shape and once to label them. This is deliberate — the alternative is a marker header on `HEAD`, which would save one download per client lifetime at the cost of a contract field every implementation must agree on; the waste is bounded and self-terminating, the field is forever.
