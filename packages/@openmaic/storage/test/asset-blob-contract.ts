// Contract for the INTERNAL blob layer: `contentHash -> bytes`, where the key
// is derived from the bytes themselves. This is the layer an object-storage /
// CDN backend would replace, so its semantics get their own suite — and they
// are deliberately *not* the semantics of the outward asset store, which
// allocates ids (see `asset-contract.ts`). Content addressing is safe here
// precisely because a hash never leaves the package.
import { describe, expect, test } from 'vitest';
import type { StorageProvider } from '@openmaic/dsl';

type ReadUrl = (url: string) => Promise<Uint8Array>;

const bytes = (s: string): Uint8Array => new TextEncoder().encode(s);
// Build the Blob from the string directly (a string is a valid BlobPart). A
// `Uint8Array` BlobPart trips TS 5.7+'s `Uint8Array<ArrayBufferLike>` vs
// `ArrayBufferView<ArrayBuffer>` narrowing under the root tsconfig; the bytes
// are UTF-8 either way, so `bytes(s)` stays the source of truth for comparison.
const blob = (s: string, type = 'text/plain'): Blob => new Blob([s], { type });

export function runBlobStoreContract(
  name: string,
  makeProvider: () => StorageProvider,
  readUrl: ReadUrl,
): void {
  describe(`blob layer contract: ${name}`, () => {
    test('put returns a non-empty key', async () => {
      const p = makeProvider();
      const key = await p.put(blob('hello'));
      expect(typeof key).toBe('string');
      expect(key.length).toBeGreaterThan(0);
    });

    test('is content-addressed: identical bytes yield the same key', async () => {
      const p = makeProvider();
      const a = await p.put(blob('same-content'));
      const b = await p.put(blob('same-content'));
      expect(a).toBe(b);
    });

    test('distinct bytes yield distinct keys', async () => {
      const p = makeProvider();
      const a = await p.put(blob('one'));
      const b = await p.put(blob('two'));
      expect(a).not.toBe(b);
    });

    test('resolve yields a URL whose bytes equal the stored blob', async () => {
      const p = makeProvider();
      const key = await p.put(blob('round-trip me'));
      const url = await p.resolve(key);
      expect(url).not.toBeNull();
      expect(await readUrl(url!)).toEqual(bytes('round-trip me'));
    });

    test('concurrent resolve of the same key yields one shared URL', async () => {
      const p = makeProvider();
      const key = await p.put(blob('shared'));
      // Two in-flight resolves must not each mint a URL (the second would orphan
      // the first, which only `remove` could ever revoke). They share one.
      const [a, b] = await Promise.all([p.resolve(key), p.resolve(key)]);
      expect(a).not.toBeNull();
      expect(a).toBe(b);
    });

    test('resolve returns null for an unknown key', async () => {
      const p = makeProvider();
      expect(await p.resolve('sha256-deadbeef')).toBeNull();
    });

    test('resolve returns null after remove', async () => {
      const p = makeProvider();
      const key = await p.put(blob('temporary'));
      await p.remove(key);
      expect(await p.resolve(key)).toBeNull();
    });
  });
}
